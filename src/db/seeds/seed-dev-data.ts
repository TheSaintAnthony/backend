...
export async function seedDevData(db: NodePgDatabase<typeof schema>) {
	...
	for (const rt of roomTypeData) {
		...
		if (existing.length === 0) {
			existing = await db
				...
				.values({ name: rt.name, maxCapacity: rt.maxCapacity, isSystemManaged: true })
				.returning();
		}
	...
	if (property.length === 0) {
		property = await db
			...
			.values({
				...
				about: 'Nestled in the historic Chiado neighbourhood, The St. Anthony Hotel offers a refined blend of classic Portuguese architecture and contemporary comfort.',
				...
				checkOutTime: '11:00',
				tourismFee: '2.00',
				arrivalInstructions: 'Reception is open 24 hours. Park in Praça do Comércio and walk 5 minutes north.',
			})
			.returning();
	}
	...
	for (const room of roomsData) {
		...
		if (existing.length === 0) {
			...
			await db.insert(schema.roomPrices).values({
				roomId: newRoom.id,
				price: room.roomTypeName === 'Standard'
					? '120.00'
	...
	if (residence.length === 0) {
		residence = await db
			...
			.values({
				...
				description: 'Luxury serviced apartments for extended stays in central Lisbon.',
				about: 'Our residences combine the privacy of a private home with the services of a five-star hotel, located steps away from Lisbon\'s most iconic landmarks.',
	...
	const residenceUnitsData = [
		...
		},
		{
			...
			floor: 2,
			status: 'available',
			description: 'Spacious T2 apartment with separate living and dining areas.',
			bedroomCount: 2,
			bathroomCount: 2,
		},
		{
			...
			floor: 3,
			status: 'reserved',
			description: 'Luxury T3 apartment with premium finishes and garden access.',
			bedroomCount: 3,
			bathroomCount: 2,
		},
		{
	...
	if (restaurantBar.length === 0) {
		await db.insert(schema.restaurants).values({
			...
			description: 'A cozy cocktail bar with a curated selection of Portuguese wines and spirits.',
	...
	const jobPostingData = [
		{
			...
			description: 'Procuramos um(a) rececionista dinâmico(a) para integrar a nossa equipa. Responsabilidades incluem check-in/check-out de hóspedes, gestão de reservas e atendimento ao cliente.',
		},
		{
			...
			description: 'Oportunidade para um(a) Chefe de Cozinha experiente liderar a nossa equipa culinária e desenvolver menus inovadores com produtos regionais.',
		},
		{
			...
			description: 'Apoio às operações diárias do restaurante e bar, incluindo serviço ao cliente, preparação de mesas e suporte à equipa.',
		},
	];
